...
type ServiceStatus = "completed" | "recommended";

export function ServiceCard({ service, status }: { service: AwsService; status?: ServiceStatus }) {
  return (
    <article className="group flex h-full flex-col rounded-2xl border border-white/10 bg-white/[0.035] p-6 transition duration-300 hover:-translate-y-1 hover:border-sky-400/40 hover:bg-white/[0.06]">
      ...
      {status && <p className={`mt-3 w-fit rounded-full border px-3 py-1 text-xs font-semibold ${status === "completed" ? "border-emerald-400/30 bg-emerald-400/10 text-emerald-300" : "border-sky-400/30 bg-sky-400/10 text-sky-300"}`}>{status === "completed" ? "Ukończono" : "Rekomendowany następny moduł"}</p>}
